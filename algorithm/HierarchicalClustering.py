...
class HierarchicalClustering(QThread):
    ...
    def __init__(self,DSM,Z,metric_name,thread_id):
    ...
    def run(self):
        ...
        print(f'最佳聚类方案的 {self.metric_name} 值：', best_value)
        print('最佳聚类方案的类别标签：', best_la)
        dn = hierarchy.dendrogram(self.Z)
        # 获取簇数为 n 时的纵坐标值
        lamda = dn['icoord'][best_index][1]
        ass_coml = assembly_complexity(lamda,best_la,self.DSM)
        print('最佳聚类方案的装配复杂度：', ass_coml)
        print("-------------------------------------------------------")
        ...
        # hgsa = GeneticSimulatedAnnealing(best_la, self.DSM, compute_Sil)
    ...
    def find_best_clustering_by_D1D2(self,Z, dist_matrix): # TisaiYu[2024/6/4] 这个指标不行，可能代码写的有问题或者理解有误。
        ...
        D1D2S = []
        best_index = 0
        for num_clusters in range(len(Z)):
            cluster_labels = hierarchy.fcluster(Z, num_clusters+1, criterion='maxclust')
    ...
    def find_best_clustering_by_CE(self,Z, dist_matrix):
        ...
        for i in range(len(Z)):
            cluster_labels = hierarchy.fcluster(Z, i + 1, criterion='maxclust')
