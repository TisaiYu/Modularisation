    ...
    # TisaiYu[2024/8/30] 说明模块有不符合约束的，要进行迭代，把不符合约束的模块的零部件的DSM提取出来，再进行模块划分
        ...
        while True:
            ...
            for module_index in disobey_module_list:
                ...
                Z_new = hierarchy.linkage(DSM, method='single', metric="cosine") # TisaiYu[2024/9/12] 第一次划分应该使用ward距离识别功能决定的大部分片区域，然后使用single（最小距离）相当于识别模块间的最优接口，应该是簇间距离最小的那两个
