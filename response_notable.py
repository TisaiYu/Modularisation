...
from PyQt5.QtCore import qDebug,QThread
from PyQt5.QtWidgets import QMainWindow,QStackedWidget,QFileDialog,QTableView
...
class Modularization(QMainWindow,Ui_MainWindow): # TisaiYu[2024/5/30] 因为要在ui生成的py文件上添加槽函数，为了不在原py文件上修改（因为可能ui还需要修改，再生成新的py文件）。通过继承来添加槽函数，而不影响ui,并且由于要调用setupui，所以还要继承QMainwindow
    ...
    def classifyInputButton_clicked(self):
        ...
        self.clustering = HierarchicalClustering(self.correlation_matrices,self.correlation_weight)
        self.clustering.clustering_finished.connect(self.cluster_optimize)
        self.clustering.hierarchy_clustering()





    def cluster_optimize(self,best_cluster_labels,Z,dist_matrix):
        # TisaiYu[2024/6/17] 再画图
        self.DSM_view.axes.set_title('Correlation Matrix')
        self.DSM_view.axes.set_xlabel('Customers')
        self.DSM_view.axes.set_ylabel('distances')
        self.assessCureveLayoutG.addWidget(self.DSM_view)
        self.DSM_view.axes.imshow(dist_matrix, cmap='Blues', interpolation='nearest')
    ...
    def loadFileButton_clicked(self):
        ...
        for i in range(correlation_nums):
            ...
            self.correlation_matrices[i, :, :] = np.array(data_dict[sheet_names[index]])
        time2 = time.time()
