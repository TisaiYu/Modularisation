...
from algorithm.HierarchicalClusteringV3 import *
from algorithm.Reprod import *
...
class DrawDendrogram(QThread):
    ...
    def run(self):
        ...
        Z = hierarchy.linkage(self.DSM, method='ward', metric="euclidean")
...
class Modularization(QMainWindow,Ui_MainWindow): # TisaiYu[2024/5/30] 因为要在ui生成的py文件上添加槽函数，为了不在原py文件上修改（因为可能ui还需要修改，再生成新的py文件）。通过继承来添加槽函数，而不影响ui,并且由于要调用setupui，所以还要继承QMainwindow
    ...
    def cluster_begin(self,Z,input_sequence,dendrogram_view):
        ...
        # self.thread_SIC = HierarchicalClustering(self.DSM, Z, 'SIC', 4, AC_dict, HC_dict)
    ...
    def create_db_qt(self):
        ...
        if dbfile != '':
            ...
            self.attribute_dict = {"PartID":"PartsTable","FunctionID":"FunctionTable","ConnectionID":"ConnectionTable","ConnectPartID":"ConnectionTable","ConnectionPartID":"ConnectionTable"}
            for key,item in self.attribute_dict.items():
                query.exec_(f'''
                                INSERT INTO AttributesInfo (AttributeName,AttributeTable)
                                VALUES ('{key}','{item})
                ''')
    ...
    def update_attributes(self):
        ...
        while query.next():
            ...
            self.attribute_dict[attribute_name] = attribute_table
    ...
    def update_sql_excel_cube_create(self, table_name):
        sqltable_model = QtSql.QSqlTableModel()
        query = QtSql.QSqlQuery()
        # TisaiYu[2024/8/12] 更新添加记录表，只是记录，单元格里有逗号分割的多个值
        if table_name == "AddRecordTable":  # TisaiYu[2024/8/14] 更新除AddRecordTable和参考表的其他表
    ...
    def update_sql_excel(self,table_name):
        ...
        if table_name == "AddRecordTable":# TisaiYu[2024/8/14] 更新除AddRecordTable和参考表的其他表
            ...
            for i in range(self.SQLTableView.model().columnCount()):
                text = self.SQLTableView.model().data(self.SQLTableView.model().index(0, i))
                count = 0
                while text == '/':
                    count +=1
                    text = self.SQLTableView.model().data(self.SQLTableView.model().index(count, i))
                part_attribute = text.split(',')
                if len(part_attribute) > 1:  # TisaiYu[2024/8/14] 是多行属性
                    attr_name = self.SQLTableView.horizontalHeader().model().headerData(i, Qt.Horizontal)
                    attr_table_name = self.attribute_dict[attr_name]
            ...
            for i in range(self.SQLTableView.model().rowCount()):
                ...
                for table_name, this_table_multi_attribues_index in attribute_multirow_index.items():
                    anytext = self.SQLTableView.model().data(
                        self.SQLTableView.model().index(i, this_table_multi_attribues_index[0]))
                ...
                for table_name, this_table_multi_attribues_index in attribute_multirow_index.items():
                    ...
                    for mm in range(max_rows_count):
                        record.setValue(0, self.SQLTableView.model().data(self.SQLTableView.model().index(i, 0))) # TisaiYu[2024/8/22] partID
        ...
        if input_dialog.exec_() == QDialog.Accepted:
            ...
            if input_list[1] == "多行":
                ...
                if chinese_mtable_name not in self.table_names_dict.keys():
                    query.exec_(f'''
                                    INSERT INTO AttributesInfo (AttributeName,AttributeTable)
                                    VALUES ('{add_column_label_english}','{multi_row_table_name}')
                                                                    ''')
                ...
                query.exec_(f'''
                                                                INSERT INTO AttributesInfo (AttributeName,AttributeTable)
                                                                VALUES ('{add_column_label_english}','PartsTable')
                                                ''')
    ...
    def read_excel_to_sql(self):
        ...
        if not self.cube_create:
            self.update_sql_excel(table_name)
        else:
            self.update_sql_excel_cube_create(table_name)
    ...
    def generate_dsm_from_sql(self):
        ...
        self.DSM,self.part_info_arr = self.reprod.sql_dsm()
