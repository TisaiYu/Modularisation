...
from userInterface.ui.CustomInputDialog import *
from userInterface.ui.mainwindow_ui import *
...
class DrawDendrogram(QThread):
    ...
    def run(self):
        ...
        Z = hierarchy.linkage(dis_mat, method='single',
...
class Modularization(QMainWindow,
    ...
    """
    TisaiYu[2024/11/14] 软件界面初始化的一些函数
    ...
    """
        TisaiYu[2024/11/15] 
        数据库页的过程，包括创建，加载，增删改查用于获取关联度前数据准备
    ...
    def update_attributes(self):  # TisaiYu[2024/11/15] 增加属性，这里暂时只是有这个功能，具体实际情况可能并不是这样操作最优的，但是给予了后续可拓展的可能
    ...
    def update_form_tree(self):  # TisaiYu[2024/11/15] 更新数据库中存储各种单行多行表的信息，方便后续查找表的类型，来相应地获取数据
        query = QtSql.QSqlQuery()
        query.exec_("PRAGMA FOREIGN_KEYS=ON;")
        query.exec_('''
                                        SELECT *
    ...
    def SQLTableshow(self, index):  # TisaiYu[2024/11/15] 当界面切换不同的表名称的，这个函数用于响应显示数据库相应表的内容
    ...
    def sql_add_one_row(
            self):  # TisaiYu[2024/11/15] TODO：直接在软件界面中操作添加数据库表的一行数据（在添加记录表中），但这个函数很久远了，现在都是直接读Excel，暂时先不管这个，只是后续拓展的可能
    ...
    def update_sql_add(self, table_name):  # TisaiYu[2024/11/15] TODO：如果加了一行，其他的对应表都需要加相应内容，暂时先不管这个，只是后续拓展的可能
        sqltable_model = QtSql.QSqlTableModel()
        query = QtSql.QSqlQuery()
    ...
    def update_sql_excel_cube_create(self):  # TisaiYu[2024/11/15] TODO：要改这个，这个的管道的功能不对劲。
        type = "Cube"
        query = QtSql.QSqlQuery(self.db)
        ...
        while query.next():
            ...
            query_new.exec_(f'''SELECT o1.Function, o2.Function
            ...
            while query_new.next():
                if query_new.value(0)=='/' or query_new.value(1) == '/':
                    ...
                    sql_model.submitAll()
                    sql_model.select()
                elif query_new.value(0)!='/' and query_new.value(1) != '/':
                    function_list.append(query_new.value(0)) # TisaiYu[2024/11/19] 把管道随便和两边的任意一个组件的功能相同，因为如果管道两变
                    # function_list.append(query_new.value(1))

            function_list = set(function_list)
            for func in function_list:
                record.setValue(0, row_count)
                record.setValue(1, func)
                sql_model.insertRecord(-1, record)
    ...
    def update_sql_excel(self, table_name):  # TisaiYu[2024/11/15] 读取Excel到添加记录表后更新所有数据库表
        sqltable_model = QtSql.QSqlTableModel()
    ...
    def clear_sql_table(self):  # TisaiYu[2024/11/15] 清楚数据库当前显示的表内容
        query = QtSql.QSqlQuery()
        table_name = self.table_names_dict[self.SQLformsTree.model().itemData(self.SQLformsTree.currentIndex())[0]][0]
    ...
    def sql_delete_select_row(self):  # TisaiYu[2024/11/15] 删除添加记录一行，然后其他表也要更新。TODO：暂时也不管，用不上，只是后续拓展的可能。
    ...
    def add_attribute(
            self):  # TisaiYu[2024/11/15] 添加属性，目前是数据库添加相应属性。TODO：应该为识别相应属性，因为你要读取Excel表，就直接以Excel表就知道属性了，在Excel表里写明是哪个多行（功能还是连接）
    ...
    def delete_attribute(
            self):  # TisaiYu[2024/11/15] 删除某列属性并更新其他表。TODO:在没有加载数据时可以，但加载数据后删除属性就要删除所有表里对应的内容了。比较旧了不清楚有没有问题。后续也可以拓展的可能吧
        input_dialog = CustomInputDialog()
        if input_dialog.exec_() == QDialog.Accepted:
            input_list = input_dialog.getInputs()
            add_column_label_english = input_list[0]
    ...
    def get_all_columns(self, table_name):  # TisaiYu[2024/11/15] 获得所有列名
    ...
    def generate_query(self, main_table, multi_tables, primary_key,
                       primary_key_value=None):  # TisaiYu[2024/11/15] 这个是用来生成查询内容的
    ...
    def select_part_info(self, generate_dsm=False):  # TisaiYu[2024/11/15] 查询内容，all或者ID查询组件信息
    ...
    def read_excel_to_sql(self):  # TisaiYu[2024/11/15] 读取Excel到当前页面数据表中
    ...
    def synMovedColumn(self, logicalIndex, oldVisualIndex,
                       newVisualIndex):  # TisaiYu[2024/11/15] 移动交换表的列。TODO:交换还有点问题，现在也基本用不上。后续只是拓展的可能
        # 在这里更新数据库表的列顺序
    ...
    def export_sql_table(
            self):  # TisaiYu[2024/11/15] 将数据库的表保存为Excel。TODO:以前是因为在软件界面操作数据库，比如添加属性和值，然后再导出才会用到这个。后续只是拓展的可能
    ...
    """
    def ahpInputButton_clicked(self): # TisaiYu[2024/11/14] 输入完AHP矩阵后点击计算后执行的代码
    ...
    def setRecripocal(self, model_index):# TisaiYu[2024/11/14] 设置输入AHP的矩阵时自动填充对角线位置
    ...
    def setCorrelationTableShape(self, n): # TisaiYu[2024/11/14] 设置AHP输入矩阵的维度大小
    ...
    """
    def classifyInputButton_clicked(self):# TisaiYu[2024/11/15] 点击开始模块化划分的响应
    ...
    def reorg_dsm(self,dsm,cluster_labels,idx=0): # TisaiYu[2024/11/15] 根据模块重新排列DSM
        # 获取唯一的聚类标签并排序
        ...
        plt.show()
        if idx == 0:
            plt.savefig(f"pics/DSM重排列/初始{idx}.png",dpi=800)
        else:
            plt.savefig(f"pics/DSM重排列/迭代细分{idx}.png",dpi=800)
    def elbow_iter(self,Z): # TisaiYu[2024/11/15] 通过Elbow法获得模块数
        distances = Z[:, 2]
        if np.all(distances == distances[0]):  # TisaiYu[2024/10/28] 出现所有距离一样，那么就是要么全为一类，要么就每个都分开
            ...
            accelerations = np.pad(accelerations, (1, 1), 'constant', constant_values=(0, 0))
            max_index = np.argmax(accelerations) + 3
            module_optimum_num = len(d) + 3 - max_index

            distance = float(Z[-(module_optimum_num - 1), 2]) - 0.01
            cluster_labels = hierarchy.fcluster(Z, distance, criterion='distance')

            plt.figure(figsize=(12.8, 7.2),dpi=100)
            indexes = np.arange(1, len(d) + 1)
            plt.plot(indexes, d[::-1], marker='o', label='距离')
            plt.plot(indexes, accelerations[::-1], marker='x', label='定义的距离变化加速度')
            plt.axvline(x=module_optimum_num, color='red', linestyle='--')
            plt.text(module_optimum_num+3, 0.8, f'{module_optimum_num}', color='red',ha='center', va='bottom', fontsize=18, fontweight='bold')
            plt.xlabel('模块数量')
            plt.ylabel('值')
            plt.title('模块数量划分的分割点确定')
            plt.legend()
            plt.show()
            plt.figure()
            re = hierarchy.dendrogram(Z, color_threshold=Z[-(module_optimum_num - 1), 2],
    ...
    def metric_iter(self,Z,DSM): # TisaiYu[2024/11/15] 通过内聚度离散度的指标法获得模块数，目前只使用Sil，因为DB和CH没有提供计算距离的参数设置
        ...
        if np.all(distances == distances[0]):  # TisaiYu[2024/10/28] 出现所有距离一样，那么就是要么全为一类，要么就每个都分开
            distance = distances[0] - 0.01
            best_cluster_labels = hierarchy.fcluster(Z, distance, criterion='distance')
        else:
            for num in range(2, len(Z)-1):
                cluster_labels = hierarchy.fcluster(Z, Z[-(num-1), 2] - 1e-8, criterion='distance')
                metric_value = time_count(compute_Sil, cluster_labels, DSM)
    ...
    def cluster_begin(self, Z, input_sequence, dendrogram_view): # TisaiYu[2024/11/15] 聚类
        self.dendrogramLayout.addWidget(dendrogram_view)
        #  validate
        dis_mat = pdist(self.DSM,
                        'cosine')  # TisaiYu[2024/6/25] 层次聚类输入要么是一维数组（表示距离矩阵的压缩，比如30*30关联度矩阵，距离矩阵有30*30但是对称只取450），或者是二维数组（就是特征矩阵）
        # dis_mat = self.DSM[np.triu_indices_from(self.DSM, k=1)]
        # TisaiYu[2024/9/10] 因为功能对于很多零部件都有关联度的影响，而连接只对一些少数零部件有影响，而ward是类内方差链接法，因此对于功能这种对很多零部件有影响的大片区域数据，能够有所识别，因为功能影响大片，所以是方差稳定的主要因素，所以能先根据功能初始得到模块方案，再迭代求解
        Z = hierarchy.linkage(dis_mat, method='ward',
                              metric="cosine")  # TisaiYu[2024/9/5] 当输入是一个一维的压缩距离矩阵时（使用pdist的结果），那么metric是没有用，如果是一个二维的表示输入特征的数组，metric实际和pdist做了一样的事情
        cluster_labels = self.elbow_iter(Z)
        system_df = pd.read_excel("resources/data/SqlLoadExcel/system+cube.xlsx")
        part_name_list = system_df["PartName"].tolist()
        plt.figure(figsize=(25.6,14.4), dpi=100)
        re = hierarchy.dendrogram(Z, color_threshold=99, above_threshold_color='#bcbddc',
                                  labels=part_name_list)
        plt.xticks(fontsize=7)
        plt.subplots_adjust(left=0.02, right=0.99, top=0.95, bottom=0.15)
        plt.xlabel("部件编码")
        plt.ylabel("类间距离")
        plt.title("部件聚类树")
        plt.savefig("pics/（这个比较明显区分）部件聚类树图（Ward）.png",dpi=800)
        plt.show()

        self.reorg_dsm(self.DSM,cluster_labels,0)
        self.draw_2D_conn(cluster_labels,count=0)

        '''----------------------------------------------------------
        *这里仅用于生成甲方给的案例需要的迭代结果图片，代码和调研MEP迭代划分时类似。后续应该改为存储在文件夹中
        ----------------------------------------------------------'''
        # TisaiYu[2024/11/13] 每次迭代都显示划分结果后的排列DSM
        #TODO:可能还要加连通量的指标来判断是否停止划分，因为有一些细分了 不是整个连通的，这种要再划。
        if not self.essay_data:
            ...
            while len(unique_labels)!=self.DSM.shape[0]:
                for module_index in unique_labels:
                    ...
                    high_rank_comps = np.where(cluster_labels > unique_labels[module_index-1])[0]
                    if len(disobey_module_comps)>1:
                        ...
                        dis_mat = pdist(DSM,'cosine')
                        Z_new = hierarchy.linkage(dis_mat, method='ward', metric="cosine")
                        small_best_clustering_labels = self.elbow_iter(Z_new)
                        # small_best_clustering_labels = self.metric_iter(Z_new, DSM)
                        if len(np.unique(small_best_clustering_labels)) == len(disobey_module_comps):# TisaiYu[2024/11/19] 各自为一类，无法计算Sil，认为是可行的吧
                            continue
                        else:
                            Sil_value = compute_Sil(small_best_clustering_labels, DSM)
                        if Sil_value<0.4:
                            continue
                        count +=1
                        new_labels[disobey_module_comps] += small_best_clustering_labels-1  # TisaiYu[2024/8/30] 确保标签位移，就算不是连续也不影响分类
                        new_labels[high_rank_comps] += len(np.unique(small_best_clustering_labels))-1  # TisaiYu[2024/8/30] 确保标签位移，就算不是连续也不影响分类

                        print("Sil_Value:",Sil_value)
                        self.reorg_dsm(self.DSM, new_labels,count)
                        ...
                        **************************************************'''
                        ...
                        if len(np.unique(small_best_clustering_labels))==Z_new.shape[0]+1:# TisaiYu[2024/11/14] 表示各自为一类，实际没有划分
                            re = hierarchy.dendrogram(Z_new, color_threshold=Z_new[0,2]-0.01, above_threshold_color='#bcbddc',labels=disobey_module_comps)
                        else:
                            re = hierarchy.dendrogram(Z_new,
                                                      color_threshold=Z_new[-(len(np.unique(small_best_clustering_labels)) - 1), 2],
                                                      above_threshold_color='#bcbddc',labels=disobey_module_comps)
                        plt.xticks(fontsize=8)
                        plt.show()
                        plt.savefig(f"pics/当前需要细分模块再细分时的聚类树/迭代{count}.png",dpi=800)

                        self.draw_2D_conn(new_labels,disobey_module_comps,count)
                        '''*******************************************
                        **************************************************'''
    ...
    def calculate_pos(self, G, classified_labels):
        # 计算每个类别的子图位置
        unique_labels = np.unique(classified_labels)
        pos = {}
        width = 1  # 增大宽度
        height = 1  # 增大高度
        n = len(unique_labels)
        ...
        for i, label in enumerate(unique_labels):
            subgraph_nodes = [n for n in G.nodes() if classified_labels[n] == label]
            subgraph = G.subgraph(subgraph_nodes)
            subgraph_pos = nx.spring_layout(subgraph, k=10)  # 增大子图布局的k值
            # 计算偏移量，增加额外间隔
            offset_x = (i % int(np.sqrt(n))) * width + (i % int(np.sqrt(n)))*1.3
            offset_y = (i // int(np.sqrt(n))) * height + (i // int(np.sqrt(n)))*1.3
    ...
    def draw_2D_conn(self,classified_labels,changed_index=None,count=0):
        conn_DSM = np.array(pd.read_excel("连接关联度矩阵.xlsx",header=None,index_col=None))
        type_labels = []
        conn_part_ids = {}
        query = QtSql.QSqlQuery()
        query.exec_("SELECT Type From PartsTable")
        while query.next():
            type_labels.append(query.value(0))
        query.exec_("SELECT PartID,ConnectionPartID From AddRecordTable")
        while query.next():
            conn_part_ids[query.value(1)] = int(query.value(0))


        G = nx.Graph()

        # 定义不同类型的节点形状
        node_shapes = {'E': 's', 'T': '^', 'V': 'o'}  # E: Square, T: Triangle, V: Circle

        # 如果没有传入 changed_index，则初始化布局和颜色
        if changed_index is None:
            unique_labels = np.unique(classified_labels)
            colors = plt.cm.get_cmap('tab20', len(unique_labels)).colors
            label_color_map = {label: colors[i] for i, label in enumerate(unique_labels)}
            self.initial_colors = label_color_map
        else:
            # 使用初始颜色映射，仅更新细化类的颜色
            label_color_map = self.initial_colors.copy()

            small_unique_labels = np.unique(classified_labels[changed_index])
            unique_labels = np.unique(classified_labels)
            high_rank_uniuqe_labels = sorted([x for x in set(unique_labels) - set(small_unique_labels) if x > max(set(small_unique_labels))])

            for j, label in enumerate(high_rank_uniuqe_labels, start=np.min(small_unique_labels) + 1):
                label_color_map[label] = self.initial_colors[j]

            for i, label in enumerate(small_unique_labels):
                if i == 0:
                    pass # TisaiYu[2024/11/19] 可以有一个颜色与原本颜色相同
                else:
                    color_to_append = len(small_unique_labels)-1
                    while color_to_append>0:
                        color = np.random.rand(3)# 生成随机颜色 if color not in existing_colors: return color
                        color = np.append(color, 1.0)
                         # 判断颜色是否在字典的values中 color_exists = any(np.array_equal(color_value, color_extended) for color_value in
                        color_exists = any(np.array_equal(color_value, color) for color_value in label_color_map.values())
                        if color_exists:
                            continue
                        else:
        ...
        # 过滤出E、T、V类型的节点
        valid_types = {'E', 'T', 'V'}
        node_indices = [i for i, t in enumerate(type_labels) if t in valid_types]


        # 添加节点到图中
        for idx in node_indices:
            node_type = type_labels[idx]
            G.add_node(idx, type=node_type, color=label_color_map[classified_labels[idx]])

        # 添加边到图中，包括管道
        for i in range(conn_DSM.shape[0]):
            for j in range(conn_DSM.shape[0]):
                if i != j and conn_DSM[i][j] == 0.8:
                    conn_str = ','.join([str(i),str(j)])
                    cube_id = conn_part_ids.get(conn_str,None)
                    if cube_id == None:
                        conn_str = ','.join([str(j), str(i)])
                        cube_id = conn_part_ids.get(conn_str, None)
                    G.add_edge(i, j, color=label_color_map[classified_labels[cube_id]])

        # 绘制图
        # TisaiYu[2024/11/19] 每次都采用一样的布局，保证ID的排布不变
        if self.pos is None:
            self.pos = self.calculate_pos(G,classified_labels)
        plt.figure(figsize=(12.8, 7.2),dpi=100)

        for shape, shape_code in node_shapes.items():
            node_list = [node for node in G.nodes(data=True) if node[1]['type'] == shape]
            nx.draw_networkx_nodes(
                G, self.pos, nodelist=[n[0] for n in node_list], node_shape=shape_code,
                node_color=[n[1]['color'] for n in node_list], node_size=500
            )

        edges = G.edges(data=True)
        edge_colors = [edge[2]['color'] for edge in edges]
        nx.draw_networkx_edges(G, self.pos, edge_color=edge_colors)
        nx.draw_networkx_labels(G, self.pos)

        plt.title('MEP System Connections')
        plt.show()
        if count == 0:
            plt.savefig(f"pics/类别的颜色图/初始{count}.png",dpi=800)
        else:
            plt.savefig(f"pics/类别的颜色图/迭代细分{count}.png",dpi=800)
        print("2D Connetion Drawing is Done!")
    ...
    def generate_dsm_from_sql(self): # TisaiYu[2024/11/15] 点击按钮响应这个，获取DSM关联度矩阵
        ...
        df = pd.DataFrame(self.DSM)
        df.index = df.index + 1
        df.to_excel('DSM.xlsx', header=False,index=False)

        self.parts_num = self.DSM.shape[0]
        self.dipict_matrix('Correlation Matrix', 'index_row', 'index_col', self.DSMLayout, self.DSM)
        self.read_excel_file = True

    def get_AC_HC_dict_by_sql(self):# TisaiYu[2024/11/15] 调研MEP系统的
    ...
    """
    TisaiYu[2024/11/14] 一些通用响应
    
    """
    def progress_bar_change(self):  # TisaiYu[2024/11/14] 进度条显示
        self.count += 1
