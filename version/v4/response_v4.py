...
from version.v4.HierarchicalClusteringV4 import *
...
class DrawDendrogram(QThread):
    draw_finished = pyqtSignal(np.ndarray, list,
                               MplCanvas)  # TisaiYu[2024/6/20] 信号定义为类的属性，而不是实例属性，是因为连接要写在实例化调用函数之前，这样实例化的函数发出信号才可以被响应
    ...
    def run(self):
        ...
        re = hierarchy.dendrogram(Z, color_threshold=Z[-(4-1),2],above_threshold_color='#bcbddc',
...
class Modularization(QMainWindow,
    ...
    def __init__(self, parent=None):  # TisaiYu[2024/6/5] parent不晓得有什么用，但别人都这样写
        ...
        self.sqlinput_dialog_ui.partSQLinputInfotable.horizontalHeader().setSectionResizeMode(
    ...
    def cluster_begin(self, Z, input_sequence, dendrogram_view):
        ...
        self.thread_CE = HierarchicalClustering(self.DSM, Z, 'CH', 1)
    ...
    def cluster_optimize(self, best_cluster_labels, best_value, metric_name, values_dict, curve_id,module_constraints_index):
        ...
        self.dipict_cureve(metric_name, "class_nums", "value", curveLayout,
                           values_dict, module_constraints_index,3, 2, 80)  # TisaiYu[2024/6/21] 多线程导致还没有记录i时另一个线程就已经判断完了，导致绘制在了一个layout上。
    ...
    def dipict_cureve(self, title, xlabel, ylabel, layout, pic_array_dict, module_constraints_index,width=3, height=2, dpi=100):
        """
        :param title:
        :param xlabel:
        :param ylabel:
        ...
        pic_array = list(pic_array_dict.values())
        module_num_list = list(pic_array_dict.keys())
        ...
        canvas.axes.plot(module_num_list, pic_array)
        canvas.axes.axvline(x=module_constraints_index,color='red')

        # TisaiYu[2024/9/19] validate
        if self.validate:
            plt.figure()
            plt.plot(module_num_list,pic_array)
            plt.axvline(x=module_constraints_index, color='red',linestyle='--')
            plt.xlabel('模块数')
            plt.ylabel('值')
            plt.title(f'{title}')
            plt.show()
    ...
    def generate_dsm_from_sql(self):
        ...
        self.reprod = Reprod(self.validate)
        self.reprod.get_conn_relationship(query_all_str)
        self.DSM, self.part_info_arr, self.ACP_dict = self.reprod.sql_dsm()
        df = pd.DataFrame(self.DSM)
        df.index = df.index + 1
        df.to_excel('./data/DSM.xlsx', header=False)
